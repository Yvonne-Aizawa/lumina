...
export class WakeWordEngine {
  ...
  async load() {
    ...
    const resolver = (file) =>
      `${this.config.baseAssetUrl.replace(/\/+$/, "")}/${file}`;
    ...
    for (const keyword of this.config.keywords) {
      ...
      const windowSize =
        this._inferKeywordWindowSize(session) ??
        this.config.embeddingWindowSize;
  ...
  async start({ deviceId, gain = 1.0 } = {}) {
    ...
    const blob = new Blob([AUDIO_PROCESSOR], {
    ...
    this._debug("Microphone stream started", {
  ...
  async runWav(buffer) {
    ...
    const minRequiredSamples =
      this._embeddingWindowSize * this.config.frameSize;
    ...
    for (
      let i = 0;
      i < Math.floor(padded.length / this.config.frameSize);
      i++
    ) {
      const chunk = padded.subarray(
        i * this.config.frameSize,
        (i + 1) * this.config.frameSize,
      );
  ...
  async _runInference(chunk, isSpeechActive, emitEvents) {
    const melspecTensor = new ort.Tensor("float32", chunk, [
      1,
    ...
    while (this._melBuffer.length >= 76) {
      ...
      const embeddingFeeds = {
        [this._embeddingModel.inputNames[0]]: new ort.Tensor(
          "float32",
          flattenedMel,
          [1, 76, 32, 1],
        ),
      ...
      const newEmbedding =
        embeddingOut[this._embeddingModel.outputNames[0]].data;
      ...
      for (const name of Object.keys(this._keywordModels)) {
        ...
        const results = await keywordModel.session.run({
          [keywordModel.session.inputNames[0]]: finalInput,
        });
